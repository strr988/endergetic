...
public final class EEBiomeModifiers {
	...
	private static void register(BootstrapContext<BiomeModifier> context, String name, Supplier<? extends BiomeModifier> modifier) {
		context.register(ResourceKey.create(NeoForgeRegistries.Keys.BIOME_MODIFIERS, ResourceLocation.fromNamespaceAndPath(EndergeticExpansion.MOD_ID, name)), modifier.get());
	}
}
